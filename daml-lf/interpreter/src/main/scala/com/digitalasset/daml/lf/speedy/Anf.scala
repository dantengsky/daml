...
object Anf {
  ...
  //TODO: inline?
  def transformLets(depth: DepthA, env: Env, rhss: List[SExpr], body: SExpr, k0: K[SExpr]): Res = rhss match {
    case rhs::rhss =>
      transformExp(depth, env, rhs, { case (depth,rhs) =>
        Land(SELet(Array(rhs),
          transformLets(DepthA(depth.n + 1), trackBindings(depth,env,1), rhss, body, k0).bounce
        ))
      })
    case Nil =>
  ...
  def flattenAlts(depth: DepthA, env: Env, alts: Array[SCaseAlt]): Array[SCaseAlt] = {
    alts.map{ case SCaseAlt(pat,body0) =>
      ...
      val body = flattenExp(DepthA(depth.n + n), trackBindings(depth, env, n), body0)
      SCaseAlt(pat,body)
    }
  }
  ...
  def transformExp(depth: DepthA, env: Env, exp: SExpr, k: K[SExpr]): Res = Bounce(() => exp match {
    ...
    case SEAppGeneral(func, args) =>
      atomizeExp(depth, env, func, { case (depth, func) =>
        atomizeExps(depth, env, args.toList, { case (depth, args) =>
    ...
    case SECase(scrut, alts0) =>
      atomizeExp(depth, env, scrut, { case (depth, scrut) =>
  ...
  def atomizeExps(depth: DepthA, env: Env, exps: List[SExpr], k: K[List[AbsAtom]]): Res = exps match {
    case Nil => k(depth,Nil)
    case exp::exps =>
      Bounce(() =>
        atomizeExp(depth, env, exp, { case (depth,atom) =>
          atomizeExps(depth, env, exps, { case (depth,atoms) =>
            Bounce(() =>
              k(depth, atom::atoms)
            )
          })
  ...
  def atomizeExp(depth: DepthA, env: Env, exp: SExpr, k: K[AbsAtom]): Res = {
    exp match {
      ...
      case _ =>
        transformExp(depth, env, exp, { case (depth, anf) =>
